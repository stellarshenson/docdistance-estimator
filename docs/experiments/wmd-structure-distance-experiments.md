...
- **Refuted, do not revisit** - **GW/FGW as the order/section instrument** - GW ≈ 0 on a pure reorder is now confirmed on data, not just argued; the order signal comes from the position-aware Phase 2 + Phase 6, never GW. **Both-document common-mode rejection (E09-H49..H53)** - rejecting the pooled common mode removes shared topic and hurts both axes on diffuse pairs (every CMRR < 1); do not resurrect per-pair or pooled anisotropy removal as a structure or single-pair lever - anisotropy removal stays a corpus-only postprocess (the content track's offline route), and the ~28-statements/pair pooled estimate is too unstable (< 0.9) to trust regardless
