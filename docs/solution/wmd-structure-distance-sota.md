...
- **Programme gate still open** - the real-conversion-pair kill-gate (E07-H28: do agentic pipelines actually restructure-while-preserving-content) needs ≥10 real pairs that do not yet exist; the structure axis ships today as an experimental readout in the diff, and that gate governs promoting it out of experimental
...
- **Status** - shipped. The OPW order-gap is packaged in `src/docdistance/distance.py` (`opw_gap`, with `opw_plan` / `opw_cost` and the crisp `order_alignment` / `structure_displacement`) and surfaced as `structure_closeness` in the interpretable diff - `DocDistance.distance_with_diff` and the CLI `distance --diff-json`, which report a per-statement `semantic_gap` (meaning) and `displacement` (order) beside the top-level `smd` and `order_gap`. It ships as an experimental readout; the E07-H28 real-conversion-pair gate stays open for promoting it out of experimental
...
- **Honest status** - shipped as an experimental readout, confirmed on two articles and a back-translation paraphrase; the OPW hyperparameter selection, a broader article set, a genuine LLM-rewrite control, and the real-conversion-pair gate (E07-H28) are the open items for promoting it out of experimental
